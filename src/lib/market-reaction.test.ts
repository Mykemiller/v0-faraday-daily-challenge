...
test("par mode: fast solve → Ahead of Consensus", () => {
  // Signal Drop par = 60s; ratio < 0.5 ⇒ ahead.
  const r = resolveMarketReaction("Signal Drop", 20);
  assert.ok(r);
...
test("par mode: mid solve → On Pace", () => {
  const r = resolveMarketReaction("Signal Drop", 60); // ratio 1.0, ≤ 1.25
  assert.ok(r);
...
test("par mode: slow solve → Taking the Long View (softened copy)", () => {
  const r = resolveMarketReaction("Signal Drop", 120); // ratio 2.0 > 1.25
  assert.ok(r);
...
test("par mode: every seeded game type resolves", () => {
  for (const g of Object.keys(PAR_TIMES)) {
    const r = resolveMarketReaction(g, 10);
...
test("unknown game with no par and no band → null", () => {
  assert.equal(resolveMarketReaction("Nonexistent", 30), null);
});

test("unusable elapsed (null / negative / NaN) → null", () => {
  assert.equal(resolveMarketReaction("Rackl", null), null);
  assert.equal(resolveMarketReaction("Rackl", undefined), null);
  assert.equal(resolveMarketReaction("Rackl", -5), null);
  assert.equal(resolveMarketReaction("Rackl", Number.NaN), null);
});
...
test("bands below the sample floor are ignored → par fallback", () => {
  const thin: SolveBandMap = {
    Rackl: { p33Sec: 40, p67Sec: 80, sampleSize: MIN_SAMPLE_FOR_BANDS - 1 },
  };
  const r = resolveMarketReaction("Rackl", 30, thin);
  assert.ok(r);
...
test("malformed band (p67 < p33) is ignored → par fallback", () => {
  const bad: SolveBandMap = {
    Rackl: { p33Sec: 80, p67Sec: 40, sampleSize: 100 },
  };
  const r = resolveMarketReaction("Rackl", 30, bad);
  assert.equal(r?.source, "par");
});

test("a game type absent from bands still classifies via its par", () => {
  // bands only covers Rackl; Frequency should fall back to par.
  const r = resolveMarketReaction("Frequency", 10, bands);
  assert.equal(r?.source, "par");
  assert.equal(r?.tier, "ahead");
});

...
| `src/components/GameIcon.jsx` `GAME_ACCENT` | The 7 per-game accents + `deep` + hover glows (`GAME_NEON` is a back-compat alias) | Keep — source of truth. **Re-derived 2026-07-30** from the raster art refresh, superseding FAR-394's jewel tones (Myke-approved). Mirrored in `@theme` `--color-game-*` **and** in `scripts/contrast-check.mjs` — sync all three |
...
- Lobby bands, tiles, user-state, scorecards, and leaderboard live in `src/components/DailyChallenge.jsx` (+ `GameIcon.jsx`, `BrandMark.jsx`). The 7 game icons are no longer drawn in code — since 2026-07-30 they are raster art in `public/icons/games/`, generated by `scripts/build-game-icons.mjs`.

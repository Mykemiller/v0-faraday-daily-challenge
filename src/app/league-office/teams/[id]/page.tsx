...
import { getTeam, resolveSeasonScope } from "@/lib/league-office/data";
...
export default async function TeamDetailPage({
  ...
  // Same scope as the Teams cards, so "3 members" on the card cannot open a
  // roster of 8. team_memberships is season-keyed — see CC-LO-TEAM-COUNTS-1.0.
  const scope = await resolveSeasonScope(staff.s, season);
  const d = await getTeam(staff.s, id, scope?.id);
  ...
  return (
    <>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, margin: "12px 0 20px", flexWrap: "wrap" }}>
        ...
        <ActionButton
          ...
          extraField={{
            ...
            label: "New captain",
            // One option per PERSON. This used to be one per membership row, so
            // a three-season member appeared three times in the dropdown.
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1.3fr 1fr", gap: 16 }}>
        <Card title={`Roster · ${d.roster.length}${scope ? ` · ${scope.name}` : " · all seasons"}`}>
          ...
          {d.roster.length === 0 ? (
            ...
            <ul style={{ listStyle: "none", margin: 0, padding: 0 }}>
              {d.roster.map((m) => {
                // Both actions delete/relocate exactly ONE membership row — one
                // season. When a person holds several under "All Seasons" the
                // row cannot silently guess which; Remove asks, and Move (whose
                // single extra field is already spent on the destination) sends
                // the caller to the season selector instead of acting blind.
                const one = m.seasons.length === 1 ? m.seasons[0] : null;
                const seasonList = m.seasons.map((x) => x.seasonName).join(", ");
                return (
                  <li key={m.subscriberId} style={{ display: "flex", alignItems: "center", gap: 8, padding: "9px 0", borderTop: "1px solid var(--color-cream-line)" }}>
                    <span style={{ fontSize: 13, fontWeight: 600 }}>@{m.handle}</span>
                    {m.role === "Captain" ? <StatusChip label="Captain" tone="amber" /> : null}
                    {one ? null : (
                      <span className="font-mono" style={{ fontSize: 10, color: "#8d8375" }} title={seasonList}>
                    ...
                    {canMove && one ? (
                      <ActionButton
                        ...
                        description={`Move @${m.handle} from ${d.team!.name} to another team, keeping their season (${one.seasonName}). They are removed here and added to the destination.`}
                        confirmLabel="Move"
                        payload={{ action: "membership.move", membershipId: one.membershipId }}
                        extraField={{
                          kind: "select",
                          name: "teamId",
                      ...
                      <span style={{ fontSize: 11, color: "#8d8375" }} title={`On this team for: ${seasonList}`}>
                        Pick a season to move
                      </span>
                    ) : null}
                    {one ? (
                      <ActionButton
                        ...
                        description={`Remove @${m.handle} from ${d.team!.name} for ${one.seasonName}. That membership record is deleted; any other season stays.`}
                        confirmLabel="Remove"
                        payload={{ action: "membership.deny", membershipId: one.membershipId }}
                      />
                    ) : (
                      <ActionButton
                        label="Remove"
                        ...
                        destructive
                        title="Remove from team"
                        description={`@${m.handle} is on ${d.team!.name} for ${m.seasons.length} seasons (${seasonList}). Choose which season's membership to delete — the others are left untouched.`}
                        confirmLabel="Remove"
                        payload={{ action: "membership.deny" }}
                        extraField={{
                          kind: "select",
                          name: "membershipId",
                          label: "Season to remove",
                          options: m.seasons.map((x) => ({ value: x.membershipId, label: x.seasonName })),
                        }}
                      />
                    )}
                  </li>
            ...
            <ul style={{ listStyle: "none", margin: 0, padding: 0 }}>
              {d.pending.map((m) => (
                <li key={m.membershipId} style={{ display: "flex", alignItems: "center", gap: 8, padding: "9px 0", borderTop: "1px solid var(--color-cream-line)" }}>
                  <span style={{ fontSize: 13, fontWeight: 600 }}>@{m.handle}</span>
                  {/* One row per REQUEST, and each request is for a season —
                      so the season is named rather than deduped away. */}
                  <span className="font-mono" style={{ fontSize: 10, color: "#8d8375" }}>{m.seasonName}</span>
                  <span style={{ flex: 1 }} />
                  <ActionButton
                    label="Approve"
                    variant="primary"
